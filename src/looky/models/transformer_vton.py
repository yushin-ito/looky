...
from .embeddings import PoseConditioningEmbedding
...
class VtonAttnProcessor2_0:
    """Attention processor used typically in processing the SD3-like self-attention projections."""

    def __init__(self):
        if not hasattr(F, "scaled_dot_product_attention"):
            raise ImportError(
                "VtonAttnProcessor2_0 requires PyTorch 2.0, to use it, please upgrade PyTorch to 2.0."
            )
    ...
    def __call__(
        ...
        **kwargs,
        ...
        key = torch.cat([key, cache[0]], dim=1)
        value = torch.cat([value, cache[1]], dim=1)
...
class VtonTransformerBlock(nn.Module):
    r"""
    ...
    def __init__(
        ...
        if hasattr(F, "scaled_dot_product_attention"):
            processor = VtonAttnProcessor2_0()
        else:
            raise ValueError(
    ...
    def forward(
        ...
        if self.use_dual_attention:
            attn_output2 = self.attn2(
                hidden_states=norm_hidden_states2, cache=cache, **joint_attention_kwargs
            )
...
class VtonTransformer2DModel(
    ...
    _no_split_modules = ["VtonTransformerBlock"]
    ...
    def __init__(
        ...
        # self.pose_cond_embedding = PoseConditioningEmbedding(
        #     conditioning_embedding_channels=1536,
        # )

        self.transformer_blocks = nn.ModuleList(
            [
                VtonTransformerBlock(
                    dim=self.inner_dim,
    ...
    def forward(
        ...
        """
        The [`VtonTransformer2DModel`] forward method.
        ...
        pose_cond = self.pose_cond_embedding(pose_cond)
        hidden_states = hidden_states + pose_cond
